...
export async function saveExtensionBookmark(
  ...
  const prepared = await prepareExtensionJsonRequest({
    request,
    maxBytes: BOOKMARK_CAPTURE_LIMITS.extensionRequestBytes,
    requestLabel: "bookmark",
  ...
  try {
    const parsedRequest = extensionBookmarkRequestSchema.safeParse(
      prepared.body,
    );
...
export async function mutateExtensionBookmark(
  ...
  const authenticatedUser = prepared.user;
  try {
    const mutation = extensionBookmarkMutationSchema.parse(prepared.body);
    let createdOption:
      | {
          kind: "view";
...
export async function removeExtensionBookmark(
  ...
  const authenticatedUser = prepared.user;
  try {
    const input = extensionBookmarkDeleteSchema.parse(prepared.body);
